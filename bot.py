...
#شناسه اکانت
bot = Bot("vsmfubkbomclrusagibcpbuvzyppmfrk")
#......
#شناسه گروه
target = "g0BfWEp02b9cefd9901ab61cc9014b27"
#......
#شناسه کانال
channell = "c0BPUKW009195a07273e70f6b5519a79"
...
p = Image.open('TEXSBOT.png')
bot.sendPhoto(target, 'TEXSBOT.png', p.size,caption=  f"اکانت هوشمند با موفقیت در گروه {GAP} فعال شد 😍🕺")
			...
			try:
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								...
								if msg.get("author_object_guid") in ozv_ajbar:
									rules = open("helps_arianbot/help.txt","r",encoding='utf-8').read()
							...
							try:
								matntabligx = open("tabligh_arianbot/banerlinkdone.txt","w",encoding='utf-8').write(str(msg.get("text").strip("جوین گروه")))
								matntabligz = open("tabligh_arianbot/banerlinkdone.txt").read().split("\n")
						...
						elif msg.get("text").startswith("ابوالفضل") or msg.get("text").startswith("شاهرخ"):
							try:
							...
							try:
								matnsingzf = open("sinzan_arianbot/banerlinkdoneSINZAN.txt","w",encoding='utf-8').write(str(msg.get("text").strip("جوین گروه")))
								matnsingz = open("sinzan_arianbot/banerlinkdoneSINZAN.txt").read().split("\n")
								bot.sendMessage(target,  "✅ با موفقیت لینک گروه مورد نظر ثبت شد")
