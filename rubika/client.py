...
address_MTN = [
			'https://messengerg2c78.iranlms.ir',
			...
			'https://messengerg2c45.iranlms.ir',
			...
			'https://messengerg2c60.iranlms.ir',
			'https://messengerg2c61.iranlms.ir',
			'https://messengerg2c63.iranlms.ir',
			'https://messengerg2c67.iranlms.ir',
...
address_FILE = [
			'https://messengerg2c26.iranlms.ir',
			'https://messengerg2c75.iranlms.ir',
			'https://messengerg2c21.iranlms.ir',
			'https://messengerg2c53.iranlms.ir',
			'https://messengerg2c32.iranlms.ir',
			'https://messengerg2c54.iranlms.ir',
			'https://messengerg2c68.iranlms.ir',
			'https://messengerg2c60.iranlms.ir',
			'https://messengerg2c1.iranlms.ir',
			'https://messengerg2c39.iranlms.ir'
			]
...
__copyright__ = "Copyright (C) 2022 arianabasi Team ArianBOT \n\nکانال ما: \n@arian__bot >"
__Arian__ = "ArianBOT"


class Bot:
	def __init__( self, auth ,app_name =__Arian__, displayWelcome = True ):
	...
	def _requestSendFile(self, file):
		return loads(self.enc.decrypt(post(json={"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps({
			...
			"client": clients.web
		}))},url=Bot._SendFile()).json()["data_enc"]))["data"]
	...
	def getMessages(self, chat_id,min_id):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(post(json={"api_version":"5","auth":self.auth,"data_enc":self.enc.encrypt(dumps(inData))},url=Bot._getURL()).json()["data_enc"])).get("data").get("messages")
				break
			except: continue
		...
		inData = {
			...
			"input":{
				...
				"sticker": {
				    "emoji_character": "😔",
				    "w_h_ratio": "1.0",
				    "sticker_id": "5e0c9c5c03ae0456535bb403",
				    "file": {
				      "file_id": "494573877",
				      "mime": "png",
				      "dc_id": "32",
				      "access_hash_rec": "449970287806732600832117650401",
				      "file_name": "sticker.png"
				    },
				},
				"sticker_set_id": "5e0c9ac6bcfaf77f4f1b647a"
			},
		}
	...
	def sendPoll(self,guid,SOAL,LIST):
		return loads(self.enc.decrypt(post(json={"api_version":"5","auth": self.auth,"data_enc":self.enc.encrypt(dumps({
			...
			"client": clients.web
		}))},url=Bot._SendPoll()).json()["data_enc"]))
	...
	def sendPhoto(self, chat_id, file, size=[], thumbnail=None, caption=None, message_id=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(post(json=data,url=Bot._SendFile()).json()["data_enc"]))
				break
			except: continue
	...
	def sendVoice(self, chat_id, file, time, caption=None, message_id=None):
		...
		while 1:
			try:
				return loads(self.enc.decrypt(loads(post(json=data,url=Bot._SendFile()).text)['data_enc']))
				break
			except: continue
